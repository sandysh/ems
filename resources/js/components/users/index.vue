<template>
  <div class="row">
    ...
    <delete-modal
      :data="user"
      @refresh-table="getUsers"
      v-if="deleteFormStore.showDeleteForm"
    ></delete-modal>
    <add-new-user-modal
      :entity="entity"
      :roles="roles"
      @refresh-table="getUsers"
      v-if="userStore.showUserForm"
    ></add-new-user-modal>
    <edit-user-modal
      :data="entity"
      :roles="roles"
      @refresh-table="getUsers"
      v-if="userStore.showEditUserForm"
    ></edit-user-modal>
  ...
  axios
    .get(
      `all?paginate=true&page=${
        users.pagination.page ? users.pagination.page : 1
      }`
    )
    .then((response) => {
...
function deleteUser(user) {
  ...
  user.value = { label: user.username, id: user.id, model: "User" };
}
